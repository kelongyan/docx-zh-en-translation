...
#### 方案 A：兼容 Chat Completions 风格的可配置接口（默认优先）
脚本通过环境变量读取翻译后端配置，设计上并不绑定某一个固定模型、品牌或服务商。只要你的服务能够提供**兼容 Chat Completions 风格**的 HTTP 接口，就可以接入。
...
这些名称表达的是配置语义，而不是特定厂商约定。也就是说，只要接口协议兼容，你可以将其配置为任意可用的翻译后端服务。
...
只要你的后端提供兼容 Chat Completions 风格的 HTTP 接口，就可以复用这组配置语义。

### Windows CMD

```bat
set API_BASE=https://your-api-base
set API_KEY=your_api_key
set MODEL=your_model_id
```

### PowerShell

```powershell
$env:API_BASE="https://your-api-base"
